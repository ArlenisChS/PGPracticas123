...
void GameObject::render(std::shared_ptr<GLMatrices> mats) {
	...
	mats->scale(GLMatrices::MODEL_MATRIX, glm::vec3(factor / _model->maxDimension()));
...
void MyRender::render_models() {
	...
	if (!avilable_models.empty()) {
		if (left_road.empty() || 
			left_road.back()->distanceTo(glm::vec3(-ROAD_SIZE, 0, FAR)) >= current_left_distance) {
			int pos = random(0, avilable_models.size());
			auto model = avilable_models[pos];
			model->translateX(-ROAD_SIZE);
	...
	if (!avilable_models.empty()) {
		if (right_road.empty() ||
			right_road.back()->distanceTo(glm::vec3(ROAD_SIZE, 0, FAR)) >= current_right_distance) {
			int pos = random(0, avilable_models.size());
			auto model = avilable_models[pos];
			model->translateX(ROAD_SIZE);
			model->translateZ(FAR);
